...
namespace JaBak.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email , ID_role=model.ID_role, Nom=model.Nom , Prenom=model.Prenom };
                var result = await UserManager.CreateAsync(user, model.Password);
                await Task.Delay(10000);
                //******
                await UserManager.AddToRoleAsync(user.Id, model.ID_role);
                //******
